...
int Array::Delete(int index) {
    int deletedValue = 0; // Initialize the deleted value to 0
    
    if (index >= 0 && index < length) { // Check if the index is within the bounds of the array
        deletedValue = items[index]; // Save the value of the element to be deleted 
        for (int i = index; i < length - 1; i++) { // Shift all elements after the index to the left by one position
            items[i] = items[i + 1];
        }
        length--; // Decrement the length of the array to indicate that one element has been deleted
    }
...
int main() {
    ...
    cout << "The size of the array is " << arr1->getSize() << ", and the length is " << arr1->getLength() << endl;
